...
__EOF__
    ...
    if [ "$dd_cluster_name" = "" ]; then
        ...
        if [ "$cluster_name" = "" ];then
            ...
            echo -e "$(tput setaf 3)Warning!! Can't get cluster uid, you need to specify the cluster name in $alamedascaler_cluster_filename $(tput sgr 0)"
            display_cluster_scaler_file_location
            return
        fi
    fi
    ...
    if [ "$dd_cluster_name" != "" ]; then
        ...
        if [ "$cluster_name" != "" ]; then
            echo -e "$(tput setaf 3)Use \"$cluster_name\" as the cluster name and DD_TAGS$(tput sgr 0)"
